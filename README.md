...
|---|---|---|
| **NETWORKS** | APs heard, hidden ones as `<hidden>` (SSID, BSSID, ch, **GHz**, RSSI, enc, **VENDOR**, **DEVICE**). Collapsed into **one row per physical device** by default (the 8 BSSIDs of one radio become a single `Guest +7` row, shown with its strongest BSSID in full); press **`g`** to expand to per-SSID. `SPACE` multi-selects a device (or a BSSID in flat view) | that device's transmitter(s) |
