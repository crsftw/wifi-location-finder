...
| Targets | 3 physical APs (`b8:11:4b:fc:f6:8x`, `f8:6b:d9:49:79:ax`, `f8:6b:d9:42:6a:8x`) |
...
- **8,517 forged deauths** across 8 of 25 channels, all with source
  `ff:ff:ff:ff:ff:ff` and reason code 7. Every target is a Cisco `-SR` BSSID:
  71 target BSSIDs on 33 physical SR access points.
- **Every source is a neighbouring Meraki radio** — 9 radios on 5 physical APs
  broadcasting the `HWKI-*` SSID set, matched by clustering the forged frames on
  ...
  nearest non-HWKI candidate 1–10 dB away, and the `8c:88:81` OUI is a burned-in
...
- **`MODERN-SR` is never targeted** on any channel, despite beaconing on all of
...
classifying the SR network as rogue and containing it, from mains-powered ceiling
...
**MFP-capable flag alone is not enough**: `WORKPLACE-SR`'s PMF-capable BSSIDs
(`akm=1,3`, 802.1X + FT) were still hit 610 times across 5 BSSIDs. The only SSID
never touched on any channel is `MODERN-SR`, which advertises `akm=1,5` —
...
MODERN-SR's, not merely "PMF ticked".
...
its rogue containment off, or stop it classifying the SR APs as rogues. Either
